...
async function ensureCertificatesTable() {
  try {
    ...
    `);
    await db.query("CREATE INDEX IF NOT EXISTS idx_certificates_user_id ON certificates(user_id)");

    const [columns] = await db.query(
      `SELECT COLUMN_NAME, IS_NULLABLE, UDT_NAME AS COLUMN_TYPE
    ...
    if (!existing.has("issued_at")) alters.push("ADD COLUMN issued_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP");
    if (!existing.has("expiry_date")) alters.push("ADD COLUMN expiry_date TIMESTAMP NULL");
    if (!existing.has("verified")) alters.push("ADD COLUMN verified SMALLINT DEFAULT 1");
    ...
    if (lessonId && lessonId.IS_NULLABLE === "NO") {
      await db.query("ALTER TABLE certificates ALTER COLUMN lesson_id DROP NOT NULL");
    }
  } catch (err) {
...
async function ensureExamTables() {
  try {
    ...
    `);
    await db.query("CREATE INDEX IF NOT EXISTS idx_exam_questions_exam_id ON exam_questions(exam_id)");
    await db.query("CREATE INDEX IF NOT EXISTS idx_exam_questions_created_by ON exam_questions(created_by)");
    ...
    `);
    await db.query("CREATE INDEX IF NOT EXISTS idx_exam_attempts_user_id ON exam_attempts(user_id)");
    await db.query("CREATE INDEX IF NOT EXISTS idx_exam_attempts_exam_id ON exam_attempts(exam_id)");
    await db.query("CREATE INDEX IF NOT EXISTS idx_exam_attempts_major ON exam_attempts(major)");
    ...
    if (!existing.has("created_by")) {
      await db.query("ALTER TABLE exam_questions ADD COLUMN created_by BIGINT NULL");
      await db.query("CREATE INDEX IF NOT EXISTS idx_exam_questions_created_by ON exam_questions(created_by)");
    }
  } catch (err) {
  ...
  try {
    ...
    const [exams] = await db.query(
      `SELECT id, major, title, description, pass_score, accent_color
       FROM exams
       WHERE major = ? AND is_active = TRUE
       LIMIT 1`,
      [major],
    );
